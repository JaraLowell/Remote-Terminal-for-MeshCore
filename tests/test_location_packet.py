...
def test_parse_location_basic():
    ...
    lat_micro = 37774900
    payload[10:14] = lat_micro.to_bytes(4, "little", signed=True)
    
    # Lon: -122.4194 degrees = -122419400 microdegrees (bytes 14-17, int32 LE)
    lon_micro = -122419400
    payload[14:18] = lon_micro.to_bytes(4, "little", signed=True)
    
    # Altitude: 50 metres (bytes 18-19, int16 LE)
    payload[18:20] = (50).to_bytes(2, "little", signed=True)
    
    # Speed: 150 cm/s = 1.5 m/s (bytes 20-21, uint16 LE)
    payload[20:22] = (150).to_bytes(2, "little", signed=False)
    
    # Heading: 9000 centidegrees = 90.00 degrees (bytes 22-23, uint16 LE)
    payload[22:24] = (9000).to_bytes(2, "little", signed=False)
    
    # Satellites: 8 (byte 24)
    payload[24] = 8
    
    # Battery: 3700 mV (bytes 25-26, uint16 LE)
    payload[25:27] = (3700).to_bytes(2, "little", signed=False)
    
    # Timestamp: 1718582400 (bytes 27-30, uint32 LE)
    payload[27:31] = (1718582400).to_bytes(4, "little", signed=False)
...
def test_parse_location_with_name():
    ...
    # Location: 0,0
    payload[10:14] = (0).to_bytes(4, "little", signed=True)  # lat
    payload[14:18] = (0).to_bytes(4, "little", signed=True)  # lon
    payload[18:20] = (0).to_bytes(2, "little", signed=True)  # altitude
    payload[20:22] = (0).to_bytes(2, "little", signed=False)  # speed
    payload[22:24] = (0).to_bytes(2, "little", signed=False)  # heading
    payload[24] = 0  # satellites
    payload[25:27] = (0).to_bytes(2, "little", signed=False)  # battery
    payload[27:31] = (0).to_bytes(4, "little", signed=False)  # timestamp
...
def test_parse_location_negative_altitude():
    ...
    payload[10:14] = (31500000).to_bytes(4, "little", signed=True)
    payload[14:18] = (35500000).to_bytes(4, "little", signed=True)
    payload[18:20] = (-430).to_bytes(2, "little", signed=True)
    
    payload[20:22] = (0).to_bytes(2, "little", signed=False)
    payload[22:24] = (0).to_bytes(2, "little", signed=False)
    payload[24] = 0
    payload[25:27] = (0).to_bytes(2, "little", signed=False)
    payload[27:31] = (0).to_bytes(4, "little", signed=False)
    payload[31] = 0
...
def test_parse_location_invalid_coordinates():
    ...
    payload[10:14] = (91000000).to_bytes(4, "little", signed=True)
    payload[14:18] = (0).to_bytes(4, "little", signed=True)
...
def test_parse_location_max_name_length():
    ...
    payload[10:14] = (0).to_bytes(4, "little", signed=True)
    payload[14:18] = (0).to_bytes(4, "little", signed=True)
    payload[18:20] = (0).to_bytes(2, "little", signed=True)
    payload[20:22] = (0).to_bytes(2, "little", signed=False)
    payload[22:24] = (0).to_bytes(2, "little", signed=False)
...
def test_location_packet_decryption_info():
    ...
    payload[10:14] = (37774900).to_bytes(4, "little", signed=True)  # lat
    payload[14:18] = (-122419400).to_bytes(4, "little", signed=True)  # lon
    payload[18:20] = (50).to_bytes(2, "little", signed=True)  # altitude
    payload[20:22] = (150).to_bytes(2, "little", signed=False)  # speed (1.5 m/s)
    payload[22:24] = (9000).to_bytes(2, "little", signed=False)  # heading (90°)
    payload[24] = 8  # satellites
    payload[25:27] = (3700).to_bytes(2, "little", signed=False)  # battery
    payload[27:31] = (1718582400).to_bytes(4, "little", signed=False)  # timestamp
